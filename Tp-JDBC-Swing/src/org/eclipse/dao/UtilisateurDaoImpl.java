	...
	public Utilisateur update(Utilisateur utilisateur) {
		...
		if (e != null) {
			try {
				...
				ppps = e.prepareStatement("UPDATE Personne SET nom = ?, SET prenom = ?,  SET sexe = ?,  SET type = ? WHERE num = ?;",
						PreparedStatement.RETURN_GENERATED_KEYS);
	...
	public Utilisateur findById(int id) {
		...
		if (c != null) {
			try {
				...
				if (r.next())
					return new Utilisateur(r.getInt("num"), r.getString("nom"), r.getString("prenom"), r.getString("sexe"), r.getString("type"));
			} catch (SQLException e) {
				e.printStackTrace();
			}
		}
